...
public class HotelOrderServiceImpl implements HotelOrderService {
    ...
    private String salesman = "373617764@qq.com";
    ...
    public void saveOrder(final TbOrder order, final String env) throws Exception {
        ...
        Thread t = new Thread() {
            public void run() {
                try {
                    if (StringUtil.equals(env, "cn")) {
                        sendTemplateMail(order.getEmail(), "", "阳光尼亚美酒店提交预约", "/order.vm", param);
                    } else if (StringUtil.equals(env, "fra")) {
                        sendTemplateMail(order.getEmail(), "", "Le soleil des hôtels de niamey soumis", "/order_fra.vm", param);
                    } else {
