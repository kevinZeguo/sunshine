...
room_order.init_event = function () {
    ...
    $("#orderNumRooms").change(function () {
        ...
        for (var i = 0; i < roomNum; i++) {
            inCountHtml += "<tr>" +
                "<td style=\"width:12%; height:30px;\">客房" + (i + 1) + "：</td>" +
        ...
        submitHandler: function (form) {
            ...
            $('#orderForm').ajaxSubmit({
                ...
                success: function (json, statusText, xhr, $form) {
                    ...
                    if (json.success) {
                        window.location.href = "/result.html?orderId=" + json.obj.id;
                    } else {
                        alert("提交订单失败," + json.msg);
                    }
                },
                error: function (e, textStatus) {
                    alert("提交订单失败![" + textStatus + "]");
                    $("#subOrder").removeAttr("disabled");
                }
            });
        }
