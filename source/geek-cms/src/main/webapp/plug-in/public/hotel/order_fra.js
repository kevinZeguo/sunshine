...
room_order.init_event = function () {
    ...
    $("#orderNumRooms").change(function () {
        ...
        for (var i = 0; i < roomNum; i++) {
            inCountHtml += "<tr>" +
                "<td style=\"width:12%; height:30px;\">pièce" + (i + 1) + "：</td>" +
...
function initForm() {
    $('#orderForm').validate({
        ...
        messages: {
            username: {
                required: "* Prie dans votre nom."
            },
            email: {
                required: "* Boîte de saisie"
            }
        ...
        submitHandler: function (form) {
            ...
            $('#orderForm').ajaxSubmit({
                ...
                success: function (json, statusText, xhr, $form) {
                    ...
                    if (json.success) {
                        window.location.href = "/rfra.html?orderId=" + json.obj.id;
                    } else {
                        alert("Bons de commande par l’échec\n," + json.msg);
                    }
                },
